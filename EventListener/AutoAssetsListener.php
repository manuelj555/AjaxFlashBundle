...
class AutoAssetsListener
{
    ...
    protected function inject(Request $request, Response $response)
    {
        ...
        if (false !== $posBody = $posrFunction($content, '</body>')) {
            ...
            if ($this->pNotify) {
                ...
                $css = null;
                $scripts = "<script src=\"{$publicPath}/bundles/manuelajaxflash/js/plugin.js\"></script>";
            }
    ...
    protected function injectSticky($basePath)
    {
        $css
            = <<<HTML
<link href="{$basePath}/bundles/manuelajaxflash/vendor/sticky/styles/sticky.min.css" type="text/css" rel="stylesheet" media="screen"/>
HTML;

        $extra = trim($this->sticky, '{}[]');
        $scripts
            = <<<HTML
<script src="{$basePath}/bundles/manuelajaxflash/vendor/sticky/sticky.min.js"></script>
<script src="{$basePath}/bundles/manuelajaxflash/js/plugin.js"></script>
<script>
jQuery(function($){
...
HTML;
    ...
    protected function injectPNotify($basePath)
    {
        $css
            = <<<HTML
<link href="{$basePath}/bundles/manuelajaxflash/vendor/pnotify/pnotify.custom.min.css" type="text/css" rel="stylesheet" media="screen"/>
HTML;
        ...
        $scripts
            = <<<HTML
<script src="{$basePath}/bundles/manuelajaxflash/vendor/pnotify/pnotify.custom.min.js"></script>
<script src="{$basePath}/bundles/manuelajaxflash/js/plugin.js"></script>
<script>
jQuery(function($){
    $.ajaxFlash('*', function(message, type, title, icon){
